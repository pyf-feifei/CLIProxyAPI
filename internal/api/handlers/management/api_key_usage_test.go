...
func TestGetAPIKeyUsage_IncludesOAuthAccounts(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	gin.SetMode(gin.TestMode)
	...
	manager.MarkResult(context.Background(), coreauth.Result{AuthID: "oauth-auth", Provider: "claude", Model: "claude-4", Success: true})
	...
	entry := payload["claude"]["|user@example.com"]
	if entry.Success != 1 || entry.Failed != 0 {
		t.Fatalf("oauth totals = %d/%d, want 1/0", entry.Success, entry.Failed)
	}
}
